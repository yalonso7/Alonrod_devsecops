...
Identify the Service: Determine which cloud-native AI/ML services you are using (e.g., AWS Sagemaker, Google Vertex AI, Azure Machine Learning).
Define the Policy: Use the Prisma Cloud Investigate Tab to test your RQL against real-time data.
Map to ATLAS: Associate your custom RQL policy with a Custom Compliance Standard named "MITRE ATLAS" to track your coverage against its specific techniques.
Automate Alerts: Enable Syslog integration or Cortex XSOAR playbooks to respond immediately when an ATLAS-aligned policy is violated.


1. Model Poisoning Risk (Training Data Protection)
MITRE ATLAS Tactic: Poisoning (AML.MT0002)
Risk: Publicly accessible storage containers (S3, Azure Blobs) that might contain training data or models.
RQL (Config):

config from cloud.resource where cloud.type = 'aws' AND api.name = 'aws-s3-bucket-list' AND json.rule = "publicAccessBlockConfiguration.blockPublicPolicy is false"

2. Unauthorized Inference API Access
MITRE ATLAS Tactic: Exfiltration (AML.MT0010) / Impact (AML.MT0015)
Risk: AI models deployed on virtual machines or containers with public IP addresses (e.g., Hugging Face models, Jupyter notebooks).

network from vpc.flow_logs where protocol = 'tcp' AND dest.port = 8080 AND source.publicinternet = 'true' AND dest.resource.group = 'ai-model-serving'


3. Insecure Model Access Management
MITRE ATLAS Tactic: Privilege Escalation (AML.MT0008)
Risk: Over-permissioned IAM roles assigned to AI services (e.g., AWS Bedrock, SageMaker) that allow unauthorized model manipulation.

config from cloud.resource where cloud.type = 'aws' AND api.name = 'aws-iam-role' AND json.rule = "policy.statement[?(@.action contains 'sagemaker:*' || @.action contains 'bedrock:*')].effect contains 'Allow'"

4. Data Exfiltration via Model Outputs 
MITRE ATLAS Tactic: Exfiltration (AML.MT0010)
Risk: Unencrypted data flow from sensitive AI services.

config from cloud.resource where cloud.type = 'azure' AND api.name = 'azure-cognitive-services' AND json.rule = "properties.networkAcls.defaultAction is 'Allow'"
...
# Future optimizations and updates for SOP
