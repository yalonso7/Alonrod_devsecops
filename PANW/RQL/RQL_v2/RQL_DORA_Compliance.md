...
- [Multi-Cloud (GCP, Azure, OCI, Alibaba, IBM)](#multi-cloud-gcp-azure-oci-alibaba-ibm)
...
In scope: **AWS** (primary); **GCP, Azure, Oracle Cloud (OCI), Alibaba Cloud, IBM Cloud** via [RQL_MultiCloud_Compliance.md](./RQL_MultiCloud_Compliance.md).  
...
# Multi-Cloud (GCP, Azure, OCI, Alibaba, IBM)

For the same DORA control intent on **GCP, Azure, Oracle OCI, Alibaba Cloud, and IBM Cloud**, use the [Multi-Cloud RQL Compliance Reference](./RQL_MultiCloud_Compliance.md). It provides equivalent `api.name` and `json.rule` patterns per provider.

**Example — DORA-01 (storage encryption):**

- **GCP** (Cloud Storage bucket without customer-managed encryption):
```sql
config from cloud.resource
where cloud.type = 'gcp' and api.name = 'gcloud-storage-buckets-list'
  and json.rule = "encryption.defaultKmsKeyName does not exist"
```
- **Azure** (storage account blob encryption disabled):
```sql
config from cloud.resource
...
Confirm `api.name` and JSON paths in your Prisma Cloud tenant for OCI, Alibaba, and IBM.
