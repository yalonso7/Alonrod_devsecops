...
# Only disable SSL warnings if explicitly requested (security risk)
...
class PrismaCloudWAASDeployer:
    """Handles deployment of WAAS policies to Prisma Cloud - Security Hardened"""
    ...
    def __init__(self, console_url: str, username: Optional[str] = None, 
        ...
        Args:
            ...
            username: Prisma Cloud username (prefer environment variables)
            password: Prisma Cloud password (prefer environment variables)
        ...
        if not self.username or not self.password:
            raise SecurityAwareException(
                "Credentials not provided. Set PRISMA_USERNAME and PRISMA_PASSWORD environment variables."
            )
    ...
    def _authenticate(self, username: str, password: str) -> str:
        """
        Authenticate and get API token - Security Hardened
        ...
        Raises:
            SecurityAwareException: If authentication fails
        """
    ...
    def load_yaml_policy(self, yaml_file: str) -> Dict[str, Any]:
        """
        Load YAML policy file and convert to Prisma Cloud format - Security Hardened
        ...
        Raises:
            SecurityAwareException: If file path is invalid or policy is malformed
        """
...
def main():
    """Main function - Security Hardened"""
    if len(sys.argv) < 2 or '--help' in sys.argv or '-h' in sys.argv:
        print_usage()
        sys.exit(0)
    
    # Parse arguments with security considerations
    console_url = None
    username = None
    password = None
    policy_type = None
    ...
    try:
        deployer = PrismaCloudWAASDeployer(console_url, username, password, verify_ssl=verify_ssl)
        ...
        logger.error(f"File not found: {safe_error_response(e)}")
        sys.exit(1)
    except Exception as e:
        print(f"\n❌ Error: {safe_error_response(e)}")
