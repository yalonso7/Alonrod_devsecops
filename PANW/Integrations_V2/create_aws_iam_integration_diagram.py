#!/usr/bin/env python3
"""
Create AWS IAM Integrations Architecture Diagram
Purpose: Generate a visual diagram of AWS IAM integrations with Cortex XDR, XSOAR, and Prisma Cloud
"""
...
cloudtrail_color = '#F39C12'

# Title
ax.text(5, 9.5, 'AWS IAM Integrations Architecture', 
        ha='center', va='top', fontsize=20, fontweight='bold')

# AWS IAM Source Box
aws_box = FancyBboxPatch((0.5, 6), 2, 2.5, 
                        boxstyle="round,pad=0.1", 
                        facecolor=aws_color, edgecolor='black', linewidth=2)
ax.add_patch(aws_box)
ax.text(1.75, 7.75, 'AWS IAM', ha='center', va='center', 
        fontsize=14, fontweight='bold', color='white')

# AWS IAM Components
ax.text(1.75, 7.3, '• Users', ha='center', va='center', fontsize=10, color='white')
ax.text(1.75, 7.0, '• Roles', ha='center', va='center', fontsize=10, color='white')
ax.text(1.75, 6.7, '• Policies', ha='center', va='center', fontsize=10, color='white')
ax.text(1.75, 6.4, '• Access Keys', ha='center', va='center', fontsize=10, color='white')
ax.text(1.75, 6.1, '• Groups', ha='center', va='center', fontsize=10, color='white')

# CloudTrail Box
cloudtrail_box = FancyBboxPatch((0.5, 3), 2, 1.5, 
                                boxstyle="round,pad=0.1", 
                                facecolor=cloudtrail_color, edgecolor='black', linewidth=2)
ax.add_patch(cloudtrail_box)
ax.text(1.75, 3.75, 'CloudTrail', ha='center', va='center', 
        fontsize=12, fontweight='bold', color='white')
ax.text(1.75, 3.4, 'Event Logs', ha='center', va='center', 
...
# Arrows from AWS IAM to CloudTrail
...
# Arrow from AWS IAM to Integration Layer
arrow2 = FancyArrowPatch((2.5, 7.25), (3.5, 6.5), 
                         arrowstyle='->', mutation_scale=20, 
                         color='black', linewidth=2)
ax.add_patch(arrow2)
ax.text(2.8, 6.8, 'User/Role Data', ha='left', va='center', fontsize=9,
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))

# Arrow from CloudTrail to Integration Layer
arrow3 = FancyArrowPatch((2.5, 3.75), (3.5, 5.5), 
                         arrowstyle='->', mutation_scale=20, 
                         color='black', linewidth=2)
ax.add_patch(arrow3)
ax.text(2.8, 4.6, 'Event Logs', ha='left', va='center', fontsize=9,
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))
...
# Legend
legend_elements = [
    mpatches.Patch(facecolor=aws_color, label='AWS IAM'),
    mpatches.Patch(facecolor=cloudtrail_color, label='CloudTrail'),
...
ax.text(5, 0.5, 'Data Flow: AWS IAM → Integration Layer → Security Platforms', 
        ha='center', va='center', fontsize=10, style='italic')

# Save as JPEG
plt.tight_layout()
import os
output_path = os.path.join(os.path.dirname(__file__), 'AWS_IAM_Integrations_Diagram.jpg')
