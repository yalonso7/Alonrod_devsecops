...
For custody, key management, and network security on **GCP, Azure, OCI, Alibaba, and IBM Cloud**, use the [Multi-Cloud RQL Compliance Reference](./RQL_MultiCloud_Compliance.md). Example equivalents for MICA-relevant controls:

- **GCP** — Public storage (MICA-02): buckets with `allUsers`:
```sql
config from cloud.resource
...
```
- **Azure** — Public blob access (MICA-02): storage allowing public blob access:
```sql
config from cloud.resource
...
Verify `api.name` and paths for OCI, Alibaba, and IBM in Prisma Cloud.
