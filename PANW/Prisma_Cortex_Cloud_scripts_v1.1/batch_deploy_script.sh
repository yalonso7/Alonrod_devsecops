...
################################################################################
# Prisma Cloud WAAS Batch Deployment Script
################################################################################
#
# This script automates the deployment of multiple WAAS policies across
# different environments and policy types.
#
# Usage:
#   ./batch_deploy.sh [options]
#
# Options:
...
#
# Examples:
#   # Deploy all policies to production
#   ./batch_deploy.sh -e production
#
#   # Dry run for staging
#   ./batch_deploy.sh -e staging --dry-run
#
#   # Deploy with backup
#   ./batch_deploy.sh -e production --backup --verbose
#
################################################################################
...
print_header() {
    echo -e "${BLUE}"
    echo "╔════════════════════════════════════════════════════════════════════╗"
    echo "║        Prisma Cloud WAAS Batch Deployment Tool                    ║"
    echo "╚════════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
check_requirements() {
    ...
    if [ ! -f "${SCRIPT_DIR}/deploy_waas_policy.py" ]; then
        log "ERROR" "deploy_waas_policy.py not found"
        exit 1
    fi
...
validate_environment() {
    local env="$1"  # Always quote variables
...
backup_policies() {
    local policy_type="$1"  # Always quote variables
    ...
    if python3 "${SCRIPT_DIR}/deploy_waas_policy.py" \
        "${PRISMA_CONSOLE_URL}" \
        "${PRISMA_USERNAME}" \
        "${PRISMA_PASSWORD}" \
        "${policy_type}" \
...
deploy_policy() {
    ...
    # Use exec to prevent shell injection, validate all inputs
...
send_notification() {
    local status="$1"  # Always quote variables
    local message="$2"
    
    # Validate inputs
    if ! validate_input "$status" '^[a-z]+$'; then
        log "WARN" "Invalid notification status: ${status}"
        return 1
    fi
    
    # Sanitize message (remove special characters that could break JSON)
    message=$(echo "$message" | sed 's/"/\\"/g' | sed 's/`/\\`/g' | sed 's/\$/\\$/g')
    
    # Slack notification (if webhook is configured)
    if [ -n "${SLACK_WEBHOOK_URL:-}" ]; then
        # Validate webhook URL
        if ! validate_input "${SLACK_WEBHOOK_URL}" '^https://hooks.slack.com/services/[a-zA-Z0-9/]+$'; then
            log "WARN" "Invalid SLACK_WEBHOOK_URL format, skipping notification"
            return 1
        fi
        ...
        # Use printf to safely construct JSON
        local json_payload=$(printf '{"attachments":[{"color":"%s","title":"WAAS Policy Deployment - %s","text":"%s","footer":"Prisma Cloud","ts":%d}]}' \
            "$color" "$ENVIRONMENT" "$message" "$(date +%s)")
    ...
    if [ -n "${NOTIFICATION_EMAIL:-}" ] && command -v mail &> /dev/null; then
        echo "${message}" | mail -s "WAAS Deployment - ${ENVIRONMENT} - ${status}" "${NOTIFICATION_EMAIL}" || true
    fi
}
...
main() {
    ...
    if [ ${total_failure} -eq 0 ]; then
        send_notification "success" "Successfully deployed ${total_success} WAAS policies to ${ENVIRONMENT}"
        log "INFO" "Deployment completed successfully"
        exit 0
    else
        send_notification "failure" "Deployment completed with ${total_failure} failures and ${total_success} successes"
