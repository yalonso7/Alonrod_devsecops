...
In scope: **AWS** (primary); **GCP, Azure, Oracle OCI, Alibaba Cloud, IBM Cloud** via [RQL_MultiCloud_Compliance.md](./RQL_MultiCloud_Compliance.md).  
...
For record-keeping, encryption, and access control on **GCP, Azure, OCI, Alibaba, and IBM Cloud**, use the [Multi-Cloud RQL Compliance Reference](./RQL_MultiCloud_Compliance.md). Example equivalents for FinTRAC-relevant controls:

- **GCP** — Storage versioning (FTRAC-01): buckets without versioning:
```sql
config from cloud.resource
...
```
- **Azure** — Disk encryption (FTRAC-06): unencrypted disks:
```sql
config from cloud.resource
...
Verify `api.name` and JSON paths for OCI, Alibaba, and IBM in your Prisma tenant.
