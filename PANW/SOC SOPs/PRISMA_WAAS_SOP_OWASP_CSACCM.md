...
Web Application & API Security (OWASP Top 10 and Cloud Security Alliance CCM Matrix Alignment).
...
# 6.5 YAML Configuration Deployment Methods

Important Note: Prisma Cloud Console does not support direct YAML file uploads through the UI. YAML configurations shown in this document are declarative representations of the actual configuration. You must deploy them using one of the following methods:

# Method 1: Prisma Cloud Console UI (Recommended for Initial Setup)

WAAS Policy Configuration via UI:

1. Navigate to WAAS Policies:
   ```
   Prisma Cloud Console → Defend → WAAS → Container/Host/Serverless
   ```

2. Create New Rule:
   - Click "Add rule" button
   - Configure settings manually based on YAML specifications
   - UI sections map to YAML as follows:

   | YAML Section | Console UI Location |
   |--------------|---------------------|
   | `appScope` | General → Scope |
   | `protectionMode` | General → Effect |
   | `httpProtection` | App firewall → OWASP Top 10 |
   | `apiProtection` | API protection → API definition |
   | `rateLimiting` | DoS protection → Rate limiting |
   | `botProtection` | Bot protection → Configuration |
   | `customRules` | Custom rules → Add rule |
   | `accessControl` | Network controls → IP lists |

3. Save and Apply:
   - Click "Save" to apply configuration
   - Changes take effect immediately (no restart required)

Step-by-Step UI Configuration Example:

```
1. Go to: Defend → WAAS → Container → Add rule
2. General tab:
   - Rule name: "production-api-protection"
   - Scope: Collections → Select "production"
   - Effect: "Prevent"
...
3. App firewall tab:
   ...
   - Code injection: "Prevent"
   - LFI: "Prevent"
   
4. API protection tab:
   - Enable "API protection"
   - API definition: Import OpenAPI spec or use discovered
   - Check "Validate request/response"
   - Effect: "Prevent"
...
6. Bot protection tab:
   - Enable bot protection
   - Known bots: "Allow"
   - Unknown bots: "Prevent"
   
7. Custom rules tab:
   - Add custom rules as needed
   
8. Network controls tab:
...
# Method 2: Prisma Cloud API (Recommended for Automation)

Convert YAML to JSON and Deploy via API:

```bash
#!/bin/bash
# deploy-waas-policy.sh

# 1. Convert YAML to JSON (use yq or python)
yq eval -o=json waas-policy.yaml > waas-policy.json

# 2. Authenticate to Prisma Cloud
TOKEN=$(curl -s -k -X POST \
  -H "Content-Type: application/json" \
  -d "{\"username\":\"$PRISMA_USER\",\"password\":\"$PRISMA_PASS\"}" \
  https://$PRISMA_CONSOLE/api/v1/authenticate | jq -r '.token')

# 3. Get existing policies (to check if update or create)
EXISTING_POLICY=$(curl -s -k -X GET \
  -H "Authorization: Bearer $TOKEN" \
  https://$PRISMA_CONSOLE/api/v1/policies/firewall/app/container | \
  jq '.rules[] | select(.name=="production-api-protection")')

# 4. Deploy policy (PUT for update, POST for create)
...
| Policy Type | API Endpoint | HTTP Method |
|-------------|--------------|-------------|
| Container WAAS | `/api/v1/policies/firewall/app/container` | GET/PUT |
| Host WAAS | `/api/v1/policies/firewall/app/host` | GET/PUT |
| Serverless WAAS | `/api/v1/policies/firewall/app/serverless` | GET/PUT |
| App-Embedded | `/api/v1/policies/firewall/app/app-embedded` | GET/PUT |
| Network List | `/api/v1/policies/firewall/app/network-list` | GET/PUT |

Python Script for Policy Deployment:
...
class PrismaCloudWAASDeployer:
    ...
    def load_yaml_policy(self, yaml_file):
        """Load and convert YAML policy to Prisma Cloud JSON format"""
        with open(yaml_file, 'r') as f:
            yaml_data = yaml.safe_load(f)
        
        # Convert YAML structure to Prisma Cloud API format
        return self._convert_to_prisma_format(yaml_data)
    
    def _convert_to_prisma_format(self, yaml_data):
        """Convert our YAML format to Prisma Cloud API JSON format"""
        # This is a simplified conversion - adjust based on actual API schema
        
        policy = {
            "rules": [{
                "name": yaml_data['metadata']['name'],
                "collections": self._convert_scope(yaml_data['spec']['appScope']),
                "applicationsSpec": yaml_data['spec']['appScope'].get('applications', []),
                
                # Protection settings
                "certificate": {},
                "tlsConfig": {},
                
                # HTTP Protection (OWASP)
                "sqlInjection": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('sqli', 'alert'),
                    "exceptionFields": []
                },
                "xss": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('xss', 'alert')
                },
                "attackTools": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('attackTools', 'alert')
                },
                "shellshock": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('shellshock', 'alert')
                },
                "malformedReq": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('malformedRequest', 'alert')
                },
                "cmdi": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('cmdi', 'alert')
                },
                "lfi": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('lfi', 'alert')
                },
                "codeInjection": {
                    "effect": yaml_data['spec']['rulesets'][0]['httpProtection'].get('codeInjection', 'alert')
                },
                
                # API Protection
                "apiSpec": {
                    "effect": "disable" if not yaml_data['spec']['rulesets'][0]['apiProtection']['enabled'] else "alert",
                    "endpoints": [],
                    "paths": []
                },
                
                # Rate limiting
                "rateLimiting": self._convert_rate_limiting(
                    yaml_data['spec']['rulesets'][0].get('rateLimiting', {})
                ),
                ...
                "botProtectionSpec": {
                    ...
                    "knownBotProtectionsSpec": {
                        "searchEngineCrawlers": "allow",
                        "businessAnalytics": "allow",
                        "educational": "allow",
                        "news": "allow",
                        "financial": "allow",
                        "contentFeedClients": "allow"
                    },
                    "unknownBotProtectionSpec": {
                        "generic": "alert",
                        "webAutomationTools": "prevent",
                        "webScrapers": "prevent",
                        "apiLibraries": "alert",
                        "httpLibraries": "alert"
                    },
                ...
                # Notes and metadata
                "notes": yaml_data['metadata'].get('description', ''),
    ...
    def _convert_scope(self, app_scope):
        """Convert appScope to Prisma collections format"""
        collections = []
        if 'applications' in app_scope:
            for app in app_scope['applications']:
                collections.append({"name": app})
        return collections
    
    def _convert_rate_limiting(self, rate_limiting):
        """Convert rate limiting configuration"""
        if not rate_limiting.get('enabled', False):
            return {"enabled": False}
        
        return {
            "enabled": True,
            "rate": rate_limiting.get('limit', 100),
            "burstSize": rate_limiting.get('burst', 150),
            "expirationTime": 60  # seconds
        }
    
    def deploy_policy(self, policy_type, policy_data):
        ...
        # Merge or replace rules
        policy_name = policy_data['rules'][0]['name']
        rule_exists = False
        
        for i, rule in enumerate(existing_policy.get('rules', [])):
    ...
    def verify_deployment(self, policy_type, policy_name):
...
def main():
    if len(sys.argv) < 6:
        print("Usage: python deploy_waas_policy.py <console_url> <username> <password> <policy_type> <yaml_file>")
        print("Policy types: container, host, serverless, app-embedded")
        sys.exit(1)
    ...
    print(f"Deploying {policy_type} WAAS policy...")
    ...
    if success:
        ...
        print(f"✗ Failed to verify policy deployment")
        sys.exit(1)
...
python deploy_waas_policy.py \
  ...
  $PASSWORD \
  serverless \
  serverless-waas-policy.yaml
```

# Method 3: Terraform Provider (Infrastructure as Code)
...
provider "prismacloud" {
  url      = var.prisma_cloud_url
  username = var.prisma_username
  password = var.prisma_password
}

# Define WAAS policy for containers
resource "prismacloud_waas_policy" "production_api" {
  rule_name = "production-api-protection"
  ...
  xss          = "prevent"
  cmdi         = "prevent"
  code_injection = "prevent"
  lfi          = "prevent"
  
  # API Protection
  api_protection_enabled = true
  api_spec_effect       = "prevent"
...
# Output policy ID
output "waas_policy_id" {
  value = prismacloud_waas_policy.production_api.id
}
```
...
# Method 4: GitOps with CI/CD Pipeline
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Deploy WAAS policies
        env:
          PRISMA_URL: ${{ secrets.PRISMA_CLOUD_URL }}
          PRISMA_USER: ${{ secrets.PRISMA_USERNAME }}
          PRISMA_PASS: ${{ secrets.PRISMA_PASSWORD }}
        run: |
          for policy in policies/waas//*.yaml; do
            echo "Deploying $policy"
            python scripts/deploy_waas_policy.py \
              $PRISMA_URL \
              $PRISMA_USER \
              $PRISMA_PASS \
              container \
      ...
      - name: Verify deployment
        run: |
          python scripts/verify_waas_policies.py
      ...
      - name: Notify team
        ...
        with:
          status: ${{ job.status }}
          text: 'WAAS policies deployed successfully'
          webhook_url: ${{ secrets.SLACK_WEBHOOK }}
```

# Method 5: twistcli Command Line Tool

```bash
# Download twistcli
curl -k -u admin:password \
  https://console.prismacloud.io/api/v1/util/twistcli \
  > twistcli && chmod +x twistcli

# Deploy WAAS policy (Note: twistcli has limited WAAS support)
# Better to use API directly for WAAS policies

# But useful for image scanning and defender deployment
./twistcli defender export kubernetes \
  --address https://console.prismacloud.io \
  --cluster-address twistlock-console
```

# Best Practices for Policy Deployment

1. Version Control: Store all policies in Git
2. Testing: Always test in non-production first
3. Validation: Validate JSON/YAML before deployment
4. Backup: Export existing policies before updates
5. Monitoring: Monitor for issues after deployment
6. Rollback Plan: Keep previous policy versions
7. Documentation: Document all changes in commit messages
8. Automation: Use CI/CD for consistent deployments

# Quick Reference: Where to Configure What

| Configuration | Console Path | API Endpoint |
|---------------|--------------|--------------|
| WAAS Rules | Defend → WAAS → Container/Host/Serverless | `/api/v1/policies/firewall/app/*` |
| API Specs | Defend → WAAS → API Observations | `/api/v1/api-specs` |
| Custom Rules | Defend → WAAS → [Rule] → Custom rules | Included in policy API |
| Network Lists | Defend → WAAS → Network list | `/api/v1/policies/firewall/app/network-list` |
| Collections | Manage → Collections and Tags | `/api/v1/collections` |
| Intelligence | Manage → System → Intelligence | `/api/v1/settings/intelligence` |
